...
type EKSConfigSpec struct {
	Mandatory cascade.EKSConfigSection `json:"mandatory,omitempty"`
	Defaults  cascade.EKSConfigSection `json:"defaults,omitempty"`
}

type EKSConfigStatus struct {
	EffectiveConfig    EffectiveEKSConfig `json:"effectiveConfig,omitempty"`
...
type EffectiveEKSConfig struct {
	...
	Mandatory cascade.EffectiveEKSSection `json:"mandatory,omitempty"`
	Defaults  cascade.EffectiveEKSSection `json:"defaults,omitempty"`
}
...
func (in *EKSConfig) DeepCopyInto(out *EKSConfig) {
	*out = *in
	...
	// Spec: *bool pointer values are safe shallow copies (values never mutated).
	out.Spec = in.Spec
	if in.Spec.Mandatory.LoggingTypes != nil {
		out.Spec.Mandatory.LoggingTypes = make([]string, len(in.Spec.Mandatory.LoggingTypes))
		copy(out.Spec.Mandatory.LoggingTypes, in.Spec.Mandatory.LoggingTypes)
	}
	...
	if in.Spec.Defaults.LoggingTypes != nil {
		out.Spec.Defaults.LoggingTypes = make([]string, len(in.Spec.Defaults.LoggingTypes))
		copy(out.Spec.Defaults.LoggingTypes, in.Spec.Defaults.LoggingTypes)
	}
	...
	if in.Status.EffectiveConfig.Mandatory.LoggingTypes != nil {
		out.Status.EffectiveConfig.Mandatory.LoggingTypes = make([]string, len(in.Status.EffectiveConfig.Mandatory.LoggingTypes))
		copy(out.Status.EffectiveConfig.Mandatory.LoggingTypes, in.Status.EffectiveConfig.Mandatory.LoggingTypes)
	}
	...
	if in.Status.EffectiveConfig.Defaults.LoggingTypes != nil {
		out.Status.EffectiveConfig.Defaults.LoggingTypes = make([]string, len(in.Status.EffectiveConfig.Defaults.LoggingTypes))
		copy(out.Status.EffectiveConfig.Defaults.LoggingTypes, in.Status.EffectiveConfig.Defaults.LoggingTypes)
	}
...
func (in *EKSConfigList) DeepCopyInto(out *EKSConfigList) {
	*out = *in
	...
	if in.Items != nil {
		out.Items = make([]EKSConfig, len(in.Items))
...
// (KRO-1199).
func (s EKSConfigStatus) MarshalJSON() ([]byte, error) {
