...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.ECSConfig) (bool, ctrl.Result, error) {
	...
	globalECSCfg, err := r.loadECSConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list ECS configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list ECS configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForECSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.ECSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list ECS configs for global ECSConfig change")
		return nil
	}
