...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.BedrockConfig{}).
		Watches(
			&v1alpha1.BedrockConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForBedrockConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.BedrockConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each BedrockKropathSection with the tier-level tags so they flow
	// through MergeBedrockCascade alongside the family-specific fields.
	globalKropathMandatoryBedrock := globalKropath.Spec.Mandatory.Bedrock
	globalKropathMandatoryBedrock.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryBedrock := localKropath.Spec.Mandatory.Bedrock
	localKropathMandatoryBedrock.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsBedrock := localKropath.Spec.Defaults.Bedrock
	localKropathDefaultsBedrock.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsBedrock := globalKropath.Spec.Defaults.Bedrock
	globalKropathDefaultsBedrock.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeBedrockCascade(
		globalKropathMandatoryBedrock, // level 1
		localKropathMandatoryBedrock,  // level 2
		globalBedrock.Spec.Mandatory,  // level 3
		cfg.Spec.Mandatory,            // level 4
		cfg.Spec.Defaults,             // level 6
		globalBedrock.Spec.Defaults,   // level 7
		localKropathDefaultsBedrock,   // level 8
		globalKropathDefaultsBedrock,  // level 9
	)
	...
	newEffConfig := v1alpha1.EffectiveBedrockConfigStatus{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.BedrockConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveBedrockConfigStatus
...
func (r *Reconciler) requestsForBedrockConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.BedrockConfig)
	if !ok {
		return nil
	}
