...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.StepFunctionsConfig) (bool, ctrl.Result, error) {
	...
	globalSFN, err := r.loadStepFunctionsConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list StepFunctionsConfigs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list StepFunctionsConfigs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForStepFunctionsConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	cfg, ok := obj.(*v1alpha1.StepFunctionsConfig)
	...
	var list v1alpha1.StepFunctionsConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list StepFunctionsConfigs for global StepFunctionsConfig change")
		return nil
	}
