...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.MemoryDBConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("MemoryDBConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.MemoryDBConfig{}).
		Watches(
			&v1alpha1.MemoryDBConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForMemoryDBConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.MemoryDBConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalMDB, globalMDBFound, globalMDBViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.MemoryDBConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("MemoryDBConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each MemoryDBKropathSection with the tier-level tags so they
	// flow through MergeMemoryDBCascade (mirrors the ElastiCache tag augmentation pattern).
	globalKropathMandatoryMDB := globalKropath.Spec.Mandatory.MemoryDB
	globalKropathMandatoryMDB.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryMDB := localKropath.Spec.Mandatory.MemoryDB
	localKropathMandatoryMDB.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsMDB := localKropath.Spec.Defaults.MemoryDB
	localKropathDefaultsMDB.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsMDB := globalKropath.Spec.Defaults.MemoryDB
	globalKropathDefaultsMDB.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeMemoryDBCascade(
		globalKropathMandatoryMDB,
		localKropathMandatoryMDB,
		globalMDB.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalMDB.Spec.Defaults,
		localKropathDefaultsMDB,
		globalKropathDefaultsMDB,
	)

	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalMDBFound, globalMDBViaFallthrough, cfg.Generation, now)

	// Cross-field validation: nodeType must be in allowedNodeTypes when both are set.
	valid, validMsg := cascade.ValidateMemoryDBNodeType(eff.Mandatory)
	...
	if !valid {
		...
		validCond.Reason = "InvalidNodeTypeNotInAllowedList"
		validCond.Message = validMsg
	}

	if !valid {
		// Validation failure: update conditions but clear effectiveConfig.
		// A stale effectiveConfig must not be consumed by RGDs after a constraint violation.
		emptyEffConfig := v1alpha1.EffectiveMemoryDBConfig{
	...
	newEffConfig := v1alpha1.EffectiveMemoryDBConfig{
		AWS:       placement.Identity,
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.MemoryDBConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveMemoryDBConfig
...
func (r *Reconciler) requestsForMemoryDBConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.MemoryDBConfig)
	if !ok {
		return nil
	}
