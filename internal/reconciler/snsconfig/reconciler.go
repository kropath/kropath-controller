...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.SNSConfig{}).
		Watches(
			&v1alpha1.SNSConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForSNSConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.SNSConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each SNSKropathSection with the tier-level tags so they flow
	// through MergeSNSCascade (mirrors the SQS/KMS tag augmentation pattern).
	globalKropathMandatorySNS := globalKropath.Spec.Mandatory.SNS
	globalKropathMandatorySNS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatorySNS := localKropath.Spec.Mandatory.SNS
	localKropathMandatorySNS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsSNS := localKropath.Spec.Defaults.SNS
	localKropathDefaultsSNS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsSNS := globalKropath.Spec.Defaults.SNS
	globalKropathDefaultsSNS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeSNSCascade(
		globalKropathMandatorySNS,
		localKropathMandatorySNS,
		globalSNS.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalSNS.Spec.Defaults,
		localKropathDefaultsSNS,
		globalKropathDefaultsSNS,
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.SNSConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveSNSConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.SNSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SNSConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForSNSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.SNSConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.SNSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SNSConfig configs for SNSConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list SNSConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
