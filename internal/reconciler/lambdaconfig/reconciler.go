...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.LambdaConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("LambdaConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.LambdaConfig{}).
		Watches(
			&v1alpha1.LambdaConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForLambdaConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.LambdaConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalLambdaCfg, globalLambdaCfgFound, globalLambdaCfgViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.LambdaConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("LambdaConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each LambdaKropathSection with the tier-level tags, syncedLabels, and
	// syncedAnnotations so they flow through MergeLambdaCascade alongside Lambda-specific fields.
	globalKropathMandatoryLambda := globalKropath.Spec.Mandatory.Lambda
	globalKropathMandatoryLambda.Tags = globalKropath.Spec.Mandatory.Tags
	globalKropathMandatoryLambda.SyncedLabels = globalKropath.Spec.Mandatory.SyncedLabels
	globalKropathMandatoryLambda.SyncedAnnotations = globalKropath.Spec.Mandatory.SyncedAnnotations
	localKropathMandatoryLambda := localKropath.Spec.Mandatory.Lambda
	localKropathMandatoryLambda.Tags = localKropath.Spec.Mandatory.Tags
	localKropathMandatoryLambda.SyncedLabels = localKropath.Spec.Mandatory.SyncedLabels
	localKropathMandatoryLambda.SyncedAnnotations = localKropath.Spec.Mandatory.SyncedAnnotations
	localKropathDefaultsLambda := localKropath.Spec.Defaults.Lambda
	localKropathDefaultsLambda.Tags = localKropath.Spec.Defaults.Tags
	localKropathDefaultsLambda.SyncedLabels = localKropath.Spec.Defaults.SyncedLabels
	localKropathDefaultsLambda.SyncedAnnotations = localKropath.Spec.Defaults.SyncedAnnotations
	globalKropathDefaultsLambda := globalKropath.Spec.Defaults.Lambda
	globalKropathDefaultsLambda.Tags = globalKropath.Spec.Defaults.Tags
	globalKropathDefaultsLambda.SyncedLabels = globalKropath.Spec.Defaults.SyncedLabels
	globalKropathDefaultsLambda.SyncedAnnotations = globalKropath.Spec.Defaults.SyncedAnnotations

	eff := cascade.MergeLambdaCascade(
		globalKropathMandatoryLambda,   // level 1
		localKropathMandatoryLambda,    // level 2
		globalLambdaCfg.Spec.Mandatory, // level 3
		cfg.Spec.Mandatory,             // level 4
		cfg.Spec.Defaults,              // level 6
		globalLambdaCfg.Spec.Defaults,  // level 7
		localKropathDefaultsLambda,     // level 8
		globalKropathDefaultsLambda,    // level 9
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalLambdaCfgFound, globalLambdaCfgViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveLambdaConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.LambdaConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveLambdaConfig
...
func (r *Reconciler) requestsForLambdaConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.LambdaConfig)
	if !ok {
		return nil
	}
