...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.MSKConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("MSKConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.MSKConfig{}).
		Watches(
			&v1alpha1.MSKConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForMSKConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.MSKConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalMSK, globalMSKFound, globalMSKViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.MSKConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("MSKConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each MSKKropathSection with the tier-level generic tags so they
	// flow through MergeMSKCascade (mirrors the ElastiCache/SQS tag augmentation pattern).
	globalKropathMandatoryMSK := globalKropath.Spec.Mandatory.MSK
	globalKropathMandatoryMSK.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryMSK := localKropath.Spec.Mandatory.MSK
	localKropathMandatoryMSK.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsMSK := localKropath.Spec.Defaults.MSK
	localKropathDefaultsMSK.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsMSK := globalKropath.Spec.Defaults.MSK
	globalKropathDefaultsMSK.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeMSKCascade(
		globalKropathMandatoryMSK,
		localKropathMandatoryMSK,
		globalMSK.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalMSK.Spec.Defaults,
		localKropathDefaultsMSK,
		globalKropathDefaultsMSK,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalMSKFound, globalMSKViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveMSKConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.MSKConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveMSKConfig
...
func (r *Reconciler) requestsForMSKConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.MSKConfig)
	if !ok {
		return nil
	}
