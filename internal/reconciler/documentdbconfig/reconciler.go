...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.DocumentDBConfig{}).
		Watches(
			&v1alpha1.DocumentDBConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForDocumentDBConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.DocumentDBConfig) (bool, ctrl.Result, error) {
	...
	// Augment each DocumentDBKropathSection with the tier-level tags so they
	// flow through MergeDocumentDBCascade (same pattern as RDS/MemoryDB tag augmentation).
	globalKropathMandatoryDocDB := globalKropath.Spec.Mandatory.DocumentDB
	globalKropathMandatoryDocDB.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryDocDB := localKropath.Spec.Mandatory.DocumentDB
	localKropathMandatoryDocDB.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsDocDB := localKropath.Spec.Defaults.DocumentDB
	localKropathDefaultsDocDB.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsDocDB := globalKropath.Spec.Defaults.DocumentDB
	globalKropathDefaultsDocDB.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeDocumentDBCascade(
		globalKropathMandatoryDocDB,
		localKropathMandatoryDocDB,
		globalDocDB.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalDocDB.Spec.Defaults,
		localKropathDefaultsDocDB,
	...
	if !valid {
		...
		validCond.Reason = "InvalidInstanceClassNotInAllowedList"
...
func (r *Reconciler) requestsForDocumentDBConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.DocumentDBConfig)
	if !ok {
		return nil
	}
