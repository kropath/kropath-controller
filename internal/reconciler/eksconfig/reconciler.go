...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EKSConfig) (bool, ctrl.Result, error) {
	...
	globalEKS, err := r.loadEKSConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) loadEKSConfig(ctx context.Context, namespace, name string) (*v1alpha1.EKSConfig, error) {
	cfg := &v1alpha1.EKSConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("EKSConfig"))
	if err := r.Client.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, cfg); err != nil {
		if client.IgnoreNotFound(err) == nil {
			return &v1alpha1.EKSConfig{}, nil
		}
		return nil, err
	}
	return cfg, nil
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list EKS configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list EKS configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForEKSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.EKSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EKS configs for global EKSConfig change")
		return nil
	}
