...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.MWAAConfig{}).
		Watches(
			&v1alpha1.MWAAConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForMWAAConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.MWAAConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalMWAA, globalMWAAFound, globalMWAAViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.MWAAConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("MWAAConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each MWAAKropathSection with the tier-level tags so that the full
	// org-wide tag set flows through MergeMWAACascade alongside MWAA-specific fields.
	globalKropathMandatoryMWAA := globalKropath.Spec.Mandatory.MWAA
	globalKropathMandatoryMWAA.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryMWAA := localKropath.Spec.Mandatory.MWAA
	localKropathMandatoryMWAA.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsMWAA := localKropath.Spec.Defaults.MWAA
	localKropathDefaultsMWAA.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsMWAA := globalKropath.Spec.Defaults.MWAA
	globalKropathDefaultsMWAA.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeMWAACascade(
		globalKropathMandatoryMWAA,
		localKropathMandatoryMWAA,
		globalMWAA.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalMWAA.Spec.Defaults,
		localKropathDefaultsMWAA,
		globalKropathDefaultsMWAA,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalMWAAFound, globalMWAAViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveMWAAConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.MWAAConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveMWAAConfig
...
func (r *Reconciler) requestsForMWAAConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.MWAAConfig)
	if !ok {
		return nil
	}
