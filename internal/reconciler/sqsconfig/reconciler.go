...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.SQSConfig) (bool, ctrl.Result, error) {
	...
	globalSQS, err := r.loadSQSConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list SQS configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list SQS configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForSQSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.SQSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SQS configs for global SQSConfig change")
		return nil
	}
