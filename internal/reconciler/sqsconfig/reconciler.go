...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.SQSConfig{}).
		Watches(
			&v1alpha1.SQSConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForSQSConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.SQSConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalSQS, globalSQSFound, globalSQSViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.SQSConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("SQSConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each SQSKropathSection with the tier-level tags so they flow
	// through MergeSQSCascade (mirrors the KMS tag augmentation pattern).
	globalKropathMandatorySQS := globalKropath.Spec.Mandatory.SQS
	globalKropathMandatorySQS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatorySQS := localKropath.Spec.Mandatory.SQS
	localKropathMandatorySQS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsSQS := localKropath.Spec.Defaults.SQS
	localKropathDefaultsSQS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsSQS := globalKropath.Spec.Defaults.SQS
	globalKropathDefaultsSQS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeSQSCascade(
		globalKropathMandatorySQS,
		localKropathMandatorySQS,
		globalSQS.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalSQS.Spec.Defaults,
		localKropathDefaultsSQS,
		globalKropathDefaultsSQS,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalSQSFound, globalSQSViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveSQSConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.SQSConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveSQSConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.SQSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SQSConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForSQSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.SQSConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.SQSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SQSConfig configs for SQSConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list SQSConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
