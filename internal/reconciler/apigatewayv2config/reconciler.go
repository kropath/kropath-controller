...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.ApiGatewayV2Config{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("ApiGatewayV2Config"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.ApiGatewayV2Config{}).
		Watches(
			&v1alpha1.ApiGatewayV2Config{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForApiGatewayV2ConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.ApiGatewayV2Config) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalApigwv2, globalApigwv2Found, globalApigwv2ViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.ApiGatewayV2Config](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("ApiGatewayV2Config"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each ApiGatewayV2KropathSection with the tier-level tags so they flow
	// through MergeApiGatewayV2Cascade (mirrors the EventBridge tag augmentation pattern).
	globalKropathMandatoryApigwv2 := globalKropath.Spec.Mandatory.ApiGatewayV2
	globalKropathMandatoryApigwv2.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryApigwv2 := localKropath.Spec.Mandatory.ApiGatewayV2
	localKropathMandatoryApigwv2.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsApigwv2 := localKropath.Spec.Defaults.ApiGatewayV2
	localKropathDefaultsApigwv2.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsApigwv2 := globalKropath.Spec.Defaults.ApiGatewayV2
	globalKropathDefaultsApigwv2.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeApiGatewayV2Cascade(
		globalKropathMandatoryApigwv2,
		localKropathMandatoryApigwv2,
		globalApigwv2.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalApigwv2.Spec.Defaults,
		localKropathDefaultsApigwv2,
		globalKropathDefaultsApigwv2,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalApigwv2Found, globalApigwv2ViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveApiGatewayV2Config{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.ApiGatewayV2Config, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveApiGatewayV2Config
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.ApiGatewayV2ConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list ApiGatewayV2Config configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForApiGatewayV2ConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.ApiGatewayV2Config)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.ApiGatewayV2ConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list ApiGatewayV2Config configs for ApiGatewayV2Config change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list ApiGatewayV2Config configs for namespace change", "namespace", ns.Name)
		return nil
	}
