...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.EMRConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("EMRConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.EMRConfig{}).
		Watches(
			&v1alpha1.EMRConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForEMRConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EMRConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalEMR, globalEMRFound, globalEMRViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.EMRConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("EMRConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each EMRKropathSection with the tier-level tags so that tag cascade
	// flows through MergeEMRCascade alongside EMR-specific fields.
	globalKropathMandatoryEMR := globalKropath.Spec.Mandatory.EMR
	globalKropathMandatoryEMR.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryEMR := localKropath.Spec.Mandatory.EMR
	localKropathMandatoryEMR.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsEMR := localKropath.Spec.Defaults.EMR
	localKropathDefaultsEMR.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsEMR := globalKropath.Spec.Defaults.EMR
	globalKropathDefaultsEMR.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeEMRCascade(
		globalKropathMandatoryEMR,
		localKropathMandatoryEMR,
		globalEMR.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalEMR.Spec.Defaults,
		localKropathDefaultsEMR,
		globalKropathDefaultsEMR,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalEMRFound, globalEMRViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveEMRConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.EMRConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveEMRConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.EMRConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EMRConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForEMRConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.EMRConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.EMRConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EMRConfig configs for EMRConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list EMRConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
