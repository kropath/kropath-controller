...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.KinesisConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("KinesisConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.KinesisConfig{}).
		Watches(
			&v1alpha1.KinesisConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForKinesisConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.KinesisConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalKinesis, globalKinesisFound, globalKinesisViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.KinesisConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("KinesisConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each KinesisKropathSection with the tier-level generic tags so they
	// flow through MergeKinesisCascade (mirrors the MSK/SQS tag augmentation pattern).
	globalKropathMandatoryKinesis := globalKropath.Spec.Mandatory.Kinesis
	globalKropathMandatoryKinesis.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryKinesis := localKropath.Spec.Mandatory.Kinesis
	localKropathMandatoryKinesis.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsKinesis := localKropath.Spec.Defaults.Kinesis
	localKropathDefaultsKinesis.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsKinesis := globalKropath.Spec.Defaults.Kinesis
	globalKropathDefaultsKinesis.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeKinesisCascade(
		globalKropathMandatoryKinesis,
		localKropathMandatoryKinesis,
		globalKinesis.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalKinesis.Spec.Defaults,
		localKropathDefaultsKinesis,
		globalKropathDefaultsKinesis,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalKinesisFound, globalKinesisViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveKinesisConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.KinesisConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveKinesisConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.KinesisConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list KinesisConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForKinesisConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.KinesisConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.KinesisConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list KinesisConfig configs for KinesisConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list KinesisConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
