...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EventBridgeConfig) (bool, ctrl.Result, error) {
	...
	globalEB, err := r.loadEventBridgeConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list EventBridge configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list EventBridge configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForEventBridgeConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	cfg, ok := obj.(*v1alpha1.EventBridgeConfig)
	...
	var list v1alpha1.EventBridgeConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EventBridge configs for global EventBridgeConfig change")
		return nil
	}
