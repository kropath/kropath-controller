...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.EventBridgeConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("EventBridgeConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.EventBridgeConfig{}).
		Watches(
			&v1alpha1.EventBridgeConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForEventBridgeConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EventBridgeConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalEB, globalEBFound, globalEBViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.EventBridgeConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("EventBridgeConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each EventBridgeKropathSection with the tier-level tags so they flow
	// through MergeEventBridgeCascade (mirrors the SNS/DynamoDB tag augmentation pattern).
	globalKropathMandatoryEB := globalKropath.Spec.Mandatory.EventBridge
	globalKropathMandatoryEB.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryEB := localKropath.Spec.Mandatory.EventBridge
	localKropathMandatoryEB.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsEB := localKropath.Spec.Defaults.EventBridge
	localKropathDefaultsEB.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsEB := globalKropath.Spec.Defaults.EventBridge
	globalKropathDefaultsEB.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeEventBridgeCascade(
		globalKropathMandatoryEB,
		localKropathMandatoryEB,
		globalEB.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalEB.Spec.Defaults,
		localKropathDefaultsEB,
		globalKropathDefaultsEB,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalEBFound, globalEBViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveEventBridgeConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.EventBridgeConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveEventBridgeConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.EventBridgeConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EventBridgeConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForEventBridgeConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.EventBridgeConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.EventBridgeConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list EventBridgeConfig configs for EventBridgeConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list EventBridgeConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
