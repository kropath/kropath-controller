...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CloudWatchLogsConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CloudWatchLogsConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.CloudWatchLogsConfig{}).
		Watches(
			&v1alpha1.CloudWatchLogsConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCloudWatchLogsConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CloudWatchLogsConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalCWL, globalCWLFound, globalCWLViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CloudWatchLogsConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CloudWatchLogsConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CloudWatchLogsKropathSection with the tier-level tags so they flow
	// through MergeCloudWatchLogsCascade (mirrors the EventBridge/DynamoDB tag augmentation pattern).
	globalKropathMandatoryCWL := globalKropath.Spec.Mandatory.CloudWatchLogs
	globalKropathMandatoryCWL.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCWL := localKropath.Spec.Mandatory.CloudWatchLogs
	localKropathMandatoryCWL.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCWL := localKropath.Spec.Defaults.CloudWatchLogs
	localKropathDefaultsCWL.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCWL := globalKropath.Spec.Defaults.CloudWatchLogs
	globalKropathDefaultsCWL.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCloudWatchLogsCascade(
		globalKropathMandatoryCWL,
		localKropathMandatoryCWL,
		globalCWL.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalCWL.Spec.Defaults,
		localKropathDefaultsCWL,
		globalKropathDefaultsCWL,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCWLFound, globalCWLViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCloudWatchLogsConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CloudWatchLogsConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCloudWatchLogsConfig
...
func (r *Reconciler) requestsForCloudWatchLogsConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CloudWatchLogsConfig)
	if !ok {
		return nil
	}
