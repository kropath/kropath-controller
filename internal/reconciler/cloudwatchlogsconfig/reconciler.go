...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CloudWatchLogsConfig) (bool, ctrl.Result, error) {
	...
	globalCWL, err := r.loadCloudWatchLogsConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) loadCloudWatchLogsConfig(ctx context.Context, namespace, name string) (*v1alpha1.CloudWatchLogsConfig, error) {
	cfg := &v1alpha1.CloudWatchLogsConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CloudWatchLogsConfig"))
	if err := r.Client.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, cfg); err != nil {
		if client.IgnoreNotFound(err) == nil {
			return &v1alpha1.CloudWatchLogsConfig{}, nil
		}
		return nil, err
	}
	return cfg, nil
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list CloudWatchLogs configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list CloudWatchLogs configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForCloudWatchLogsConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	cfg, ok := obj.(*v1alpha1.CloudWatchLogsConfig)
	...
	var list v1alpha1.CloudWatchLogsConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudWatchLogs configs for global CloudWatchLogsConfig change")
		return nil
	}
