...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.AthenaConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("AthenaConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.AthenaConfig{}).
		Watches(
			&v1alpha1.AthenaConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForAthenaConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.AthenaConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalAthena, globalAthenaFound, globalAthenaViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.AthenaConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("AthenaConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each AthenaKropathSection with the tier-level tags so they flow
	// through MergeAthenaCascade (mirrors the pattern used by EMR and CloudWatch).
	globalKropathMandatoryAthena := globalKropath.Spec.Mandatory.Athena
	globalKropathMandatoryAthena.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryAthena := localKropath.Spec.Mandatory.Athena
	localKropathMandatoryAthena.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsAthena := localKropath.Spec.Defaults.Athena
	localKropathDefaultsAthena.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsAthena := globalKropath.Spec.Defaults.Athena
	globalKropathDefaultsAthena.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeAthenaCascade(
		globalKropathMandatoryAthena,
		localKropathMandatoryAthena,
		globalAthena.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalAthena.Spec.Defaults,
		localKropathDefaultsAthena,
		globalKropathDefaultsAthena,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalAthenaFound, globalAthenaViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveAthenaConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.AthenaConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveAthenaConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.AthenaConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list AthenaConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForAthenaConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.AthenaConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.AthenaConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list AthenaConfig configs for AthenaConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list AthenaConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
