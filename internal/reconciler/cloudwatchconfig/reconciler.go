...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CloudWatchConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CloudWatchConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&v1alpha1.CloudWatchConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCloudWatchConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CloudWatchConfig) (bool, ctrl.Result, error) {
	...
	globalCW, globalCWFound, globalCWViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CloudWatchConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CloudWatchConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CloudWatchKropathSection with the tier-level tags so they flow
	// through MergeCloudWatchCascade (mirrors the cloudwatchlogs tag augmentation pattern).
	globalKropathMandatoryCW := globalKropath.Spec.Mandatory.CloudWatch
	globalKropathMandatoryCW.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCW := localKropath.Spec.Mandatory.CloudWatch
	localKropathMandatoryCW.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCW := localKropath.Spec.Defaults.CloudWatch
	localKropathDefaultsCW.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCW := globalKropath.Spec.Defaults.CloudWatch
	globalKropathDefaultsCW.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCloudWatchCascade(
		globalKropathMandatoryCW,
		localKropathMandatoryCW,
		globalCW.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalCW.Spec.Defaults,
		localKropathDefaultsCW,
		globalKropathDefaultsCW,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCWFound, globalCWViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCloudWatchConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CloudWatchConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCloudWatchConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.CloudWatchConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudWatchConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForCloudWatchConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CloudWatchConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.CloudWatchConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudWatchConfig configs for CloudWatchConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list CloudWatchConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
