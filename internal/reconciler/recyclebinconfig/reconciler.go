...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.RecycleBinConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("RecycleBinConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.RecycleBinConfig{}).
		Watches(
			&v1alpha1.RecycleBinConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForRecycleBinConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.RecycleBinConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalRB, globalRBFound, globalRBViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.RecycleBinConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("RecycleBinConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each EBSRecycleBinKropathSection with the tier-level tags so they flow
	// through MergeEBSRecycleBinCascade (mirrors the CloudWatchLogs tag augmentation pattern).
	globalKropathMandatoryRB := globalKropath.Spec.Mandatory.EBSRecycleBin
	globalKropathMandatoryRB.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryRB := localKropath.Spec.Mandatory.EBSRecycleBin
	localKropathMandatoryRB.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsRB := localKropath.Spec.Defaults.EBSRecycleBin
	localKropathDefaultsRB.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsRB := globalKropath.Spec.Defaults.EBSRecycleBin
	globalKropathDefaultsRB.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeEBSRecycleBinCascade(
		globalKropathMandatoryRB,
		localKropathMandatoryRB,
		globalRB.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalRB.Spec.Defaults,
		localKropathDefaultsRB,
		globalKropathDefaultsRB,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalRBFound, globalRBViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveRecycleBinConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.RecycleBinConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveRecycleBinConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.RecycleBinConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list RecycleBinConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForRecycleBinConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.RecycleBinConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.RecycleBinConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list RecycleBinConfig configs for RecycleBinConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list RecycleBinConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
