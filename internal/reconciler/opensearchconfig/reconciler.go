...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.OpenSearchConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("OpenSearchConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&v1alpha1.OpenSearchConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForOpenSearchConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.OpenSearchConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalOS, globalOSFound, globalOSViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.OpenSearchConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("OpenSearchConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each OpenSearchKropathSection with the tier-level tags so they
	// flow through MergeOpenSearchCascade (mirrors the ElastiCache tag augmentation pattern).
	globalKropathMandatoryOS := globalKropath.Spec.Mandatory.OpenSearch
	globalKropathMandatoryOS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryOS := localKropath.Spec.Mandatory.OpenSearch
	localKropathMandatoryOS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsOS := localKropath.Spec.Defaults.OpenSearch
	localKropathDefaultsOS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsOS := globalKropath.Spec.Defaults.OpenSearch
	globalKropathDefaultsOS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeOpenSearchCascade(
		globalKropathMandatoryOS,
		localKropathMandatoryOS,
		globalOS.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalOS.Spec.Defaults,
		localKropathDefaultsOS,
		globalKropathDefaultsOS,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalOSFound, globalOSViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveOpenSearchConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.OpenSearchConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveOpenSearchConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.OpenSearchConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list OpenSearchConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForOpenSearchConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.OpenSearchConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.OpenSearchConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list OpenSearchConfig configs for OpenSearchConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list OpenSearchConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
