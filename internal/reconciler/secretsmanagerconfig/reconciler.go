...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.SecretsManagerConfig) (bool, ctrl.Result, error) {
	...
	globalSM, err := r.loadSMConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) loadSMConfig(ctx context.Context, namespace, name string) (*v1alpha1.SecretsManagerConfig, error) {
	cfg := &v1alpha1.SecretsManagerConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("SecretsManagerConfig"))
	if err := r.Client.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, cfg); err != nil {
		if client.IgnoreNotFound(err) == nil {
			return &v1alpha1.SecretsManagerConfig{}, nil
		}
		return nil, err
	}
	return cfg, nil
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list SecretsManager configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list SecretsManager configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForSMConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	cfg, ok := obj.(*v1alpha1.SecretsManagerConfig)
	...
	var list v1alpha1.SecretsManagerConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list SecretsManager configs for global SecretsManagerConfig change")
		return nil
	}
