...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.Route53Config{}).
		Watches(
			&v1alpha1.Route53Config{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForRoute53ConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.Route53Config) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each Route53KropathSection with the tier-level tags so they flow
	// through MergeRoute53Cascade (mirrors the CloudWatchLogs/EventBridge tag augmentation pattern).
	globalKropathMandatoryR53 := globalKropath.Spec.Mandatory.Route53
	globalKropathMandatoryR53.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryR53 := localKropath.Spec.Mandatory.Route53
	localKropathMandatoryR53.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsR53 := localKropath.Spec.Defaults.Route53
	localKropathDefaultsR53.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsR53 := globalKropath.Spec.Defaults.Route53
	globalKropathDefaultsR53.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeRoute53Cascade(
		globalKropathMandatoryR53,
		localKropathMandatoryR53,
		globalR53.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalR53.Spec.Defaults,
		localKropathDefaultsR53,
		globalKropathDefaultsR53,
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.Route53Config, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveRoute53Config
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.Route53ConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list Route53Config configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForRoute53ConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.Route53Config)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.Route53ConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list Route53Config configs for Route53Config change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list Route53Config configs for namespace change", "namespace", ns.Name)
		return nil
	}
