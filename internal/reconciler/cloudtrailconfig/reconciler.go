...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CloudTrailConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CloudTrailConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.CloudTrailConfig{}).
		Watches(
			&v1alpha1.CloudTrailConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCloudTrailConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CloudTrailConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalCT, globalCTFound, globalCTViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CloudTrailConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CloudTrailConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CloudTrailKropathSection with the tier-level tags so they flow
	// through MergeCloudTrailCascade.
	globalKropathMandatoryCT := globalKropath.Spec.Mandatory.CloudTrail
	globalKropathMandatoryCT.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCT := localKropath.Spec.Mandatory.CloudTrail
	localKropathMandatoryCT.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCT := localKropath.Spec.Defaults.CloudTrail
	localKropathDefaultsCT.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCT := globalKropath.Spec.Defaults.CloudTrail
	globalKropathDefaultsCT.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCloudTrailCascade(
		globalKropathMandatoryCT,
		localKropathMandatoryCT,
		globalCT.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalCT.Spec.Defaults,
		localKropathDefaultsCT,
		globalKropathDefaultsCT,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCTFound, globalCTViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCloudTrailConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CloudTrailConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCloudTrailConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.CloudTrailConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudTrailConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForCloudTrailConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CloudTrailConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.CloudTrailConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudTrailConfig configs for CloudTrailConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list CloudTrailConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
