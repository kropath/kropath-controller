...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.OrganizationsConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("OrganizationsConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.OrganizationsConfig{}).
		Watches(
			&v1alpha1.OrganizationsConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForOrganizationsConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.OrganizationsConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalOrg, globalOrgFound, globalOrgViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.OrganizationsConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("OrganizationsConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each OrganizationsKropathSection with the tier-level tags so they flow
	// through MergeOrganizationsCascade alongside the Organizations-specific fields.
	globalKropathMandatoryOrg := globalKropath.Spec.Mandatory.Organizations
	globalKropathMandatoryOrg.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryOrg := localKropath.Spec.Mandatory.Organizations
	localKropathMandatoryOrg.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsOrg := localKropath.Spec.Defaults.Organizations
	localKropathDefaultsOrg.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsOrg := globalKropath.Spec.Defaults.Organizations
	globalKropathDefaultsOrg.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeOrganizationsCascade(
		globalKropathMandatoryOrg, // level 1
		localKropathMandatoryOrg,  // level 2
		globalOrg.Spec.Mandatory,  // level 3
		cfg.Spec.Mandatory,        // level 4
		cfg.Spec.Defaults,         // level 6
		globalOrg.Spec.Defaults,   // level 7
		localKropathDefaultsOrg,   // level 8
		globalKropathDefaultsOrg,  // level 9
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalOrgFound, globalOrgViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveOrganizationsConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.OrganizationsConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveOrganizationsConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.OrganizationsConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list OrganizationsConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForOrganizationsConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.OrganizationsConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.OrganizationsConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list OrganizationsConfig configs for OrganizationsConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list OrganizationsConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
