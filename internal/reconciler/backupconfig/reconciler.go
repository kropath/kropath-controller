...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.BackupConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("BackupConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&v1alpha1.BackupConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForBackupConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.BackupConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalBackup, globalBackupFound, globalBackupViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.BackupConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("BackupConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each BackupKropathSection with the tier-level tags:
	// BackupKropathSection carries cross-type Backup fields only; tier-level tags from
	// KropathConfigTier.Tags are promoted here so they flow through MergeBackupCascade.
	globalKropathMandatoryBackup := globalKropath.Spec.Mandatory.Backup
	globalKropathMandatoryBackup.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryBackup := localKropath.Spec.Mandatory.Backup
	localKropathMandatoryBackup.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsBackup := localKropath.Spec.Defaults.Backup
	localKropathDefaultsBackup.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsBackup := globalKropath.Spec.Defaults.Backup
	globalKropathDefaultsBackup.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeBackupCascade(
		globalKropathMandatoryBackup,
		localKropathMandatoryBackup,
		globalBackup.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalBackup.Spec.Defaults,
		localKropathDefaultsBackup,
		globalKropathDefaultsBackup,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalBackupFound, globalBackupViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveBackupConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.BackupConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveBackupConfig
...
func (r *Reconciler) requestsForBackupConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.BackupConfig)
	if !ok {
		return nil
	}
