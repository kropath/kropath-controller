...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.DSQLConfig{}).
		Watches(
			&v1alpha1.DSQLConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForDSQLConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.DSQLConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each DSQLKropathSection with the tier-level tags:
	// DSQLKropathSection carries only the DSQL cross-type field (deletionProtectionEnabled);
	// tier-level tags from KropathConfigTier.Tags are promoted here so they flow through
	// MergeDSQLCascade.
	globalKropathMandatoryDSQL := globalKropath.Spec.Mandatory.DSQL
	globalKropathMandatoryDSQL.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryDSQL := localKropath.Spec.Mandatory.DSQL
	localKropathMandatoryDSQL.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsDSQL := localKropath.Spec.Defaults.DSQL
	localKropathDefaultsDSQL.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsDSQL := globalKropath.Spec.Defaults.DSQL
	globalKropathDefaultsDSQL.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeDSQLCascade(
		globalKropathMandatoryDSQL,
		localKropathMandatoryDSQL,
		globalDSQL.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalDSQL.Spec.Defaults,
		localKropathDefaultsDSQL,
		globalKropathDefaultsDSQL,
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.DSQLConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveDSQLConfig
...
func (r *Reconciler) requestsForDSQLConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.DSQLConfig)
	if !ok {
		return nil
	}
