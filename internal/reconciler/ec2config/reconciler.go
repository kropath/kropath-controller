...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.EC2Config{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("EC2Config"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.EC2Config{}).
		Watches(
			&v1alpha1.EC2Config{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForEC2ConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EC2Config) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalEC2, globalEC2Found, globalEC2ViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.EC2Config](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("EC2Config"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each EC2KropathSection with tier-level tags/syncedLabels/syncedAnnotations
	// so they flow through MergeEC2Cascade (same pattern as RDS/KMS tag augmentation).
	globalKropathMandatoryEC2 := globalKropath.Spec.Mandatory.EC2
	globalKropathMandatoryEC2.Tags = globalKropath.Spec.Mandatory.Tags
	globalKropathMandatoryEC2.SyncedLabels = globalKropath.Spec.Mandatory.SyncedLabels
	globalKropathMandatoryEC2.SyncedAnnotations = globalKropath.Spec.Mandatory.SyncedAnnotations

	localKropathMandatoryEC2 := localKropath.Spec.Mandatory.EC2
	localKropathMandatoryEC2.Tags = localKropath.Spec.Mandatory.Tags
	localKropathMandatoryEC2.SyncedLabels = localKropath.Spec.Mandatory.SyncedLabels
	localKropathMandatoryEC2.SyncedAnnotations = localKropath.Spec.Mandatory.SyncedAnnotations

	localKropathDefaultsEC2 := localKropath.Spec.Defaults.EC2
	localKropathDefaultsEC2.Tags = localKropath.Spec.Defaults.Tags
	localKropathDefaultsEC2.SyncedLabels = localKropath.Spec.Defaults.SyncedLabels
	localKropathDefaultsEC2.SyncedAnnotations = localKropath.Spec.Defaults.SyncedAnnotations

	globalKropathDefaultsEC2 := globalKropath.Spec.Defaults.EC2
	globalKropathDefaultsEC2.Tags = globalKropath.Spec.Defaults.Tags
	globalKropathDefaultsEC2.SyncedLabels = globalKropath.Spec.Defaults.SyncedLabels
	globalKropathDefaultsEC2.SyncedAnnotations = globalKropath.Spec.Defaults.SyncedAnnotations

	eff := cascade.MergeEC2Cascade(
		globalKropathMandatoryEC2,
		localKropathMandatoryEC2,
		globalEC2.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalEC2.Spec.Defaults,
		localKropathDefaultsEC2,
		globalKropathDefaultsEC2,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalEC2Found, globalEC2ViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveEC2Config{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.EC2Config, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveEC2Config
...
func (r *Reconciler) requestsForEC2ConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.EC2Config)
	if !ok {
		return nil
	}
