...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.S3AdvancedConfig{}).
		Watches(
			&v1alpha1.S3AdvancedConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForS3AdvancedConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.S3AdvancedConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// S3AdvancedKropathSection has no Tags field — no tag-promotion step needed here.
	eff := cascade.MergeS3AdvancedCascade(
		globalKropath.Spec.Mandatory.S3Advanced, // level 1
		localKropath.Spec.Mandatory.S3Advanced,  // level 2
		globalS3AdvCfg.Spec.Mandatory,           // level 3
		cfg.Spec.Mandatory,                      // level 4
		cfg.Spec.Defaults,                       // level 6
		globalS3AdvCfg.Spec.Defaults,            // level 7
		localKropath.Spec.Defaults.S3Advanced,   // level 8
		globalKropath.Spec.Defaults.S3Advanced,  // level 9
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.S3AdvancedConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveS3AdvancedConfig
...
func (r *Reconciler) requestsForS3AdvancedConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.S3AdvancedConfig)
	if !ok {
		return nil
	}
