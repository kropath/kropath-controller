...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CognitoConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CognitoConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.CognitoConfig{}).
		Watches(
			&v1alpha1.CognitoConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCognitoConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CognitoConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalCognito, globalCognitoFound, globalCognitoViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CognitoConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CognitoConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CognitoKropathSection with tier-level tags so they flow through
	// MergeCognitoCascade alongside Cognito-specific fields (ADR-015 §3.5).
	globalKropathMandatoryCognito := globalKropath.Spec.Mandatory.Cognito
	globalKropathMandatoryCognito.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCognito := localKropath.Spec.Mandatory.Cognito
	localKropathMandatoryCognito.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCognito := localKropath.Spec.Defaults.Cognito
	localKropathDefaultsCognito.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCognito := globalKropath.Spec.Defaults.Cognito
	globalKropathDefaultsCognito.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCognitoCascade(
		globalKropathMandatoryCognito,
		localKropathMandatoryCognito,
		globalCognito.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalCognito.Spec.Defaults,
		localKropathDefaultsCognito,
		globalKropathDefaultsCognito,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCognitoFound, globalCognitoViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCognitoConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CognitoConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCognitoConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.CognitoConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CognitoConfig for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForCognitoConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CognitoConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.CognitoConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CognitoConfig for CognitoConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list CognitoConfig for namespace change", "namespace", ns.Name)
		return nil
	}
