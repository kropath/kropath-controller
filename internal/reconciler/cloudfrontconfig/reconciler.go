...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CloudFrontConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CloudFrontConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.CloudFrontConfig{}).
		Watches(
			&v1alpha1.CloudFrontConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCloudFrontConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CloudFrontConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalCFCfg, globalCFCfgFound, globalCFCfgViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CloudFrontConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CloudFrontConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CloudFrontKropathSection with the tier-level tags so they flow
	// through MergeCloudFrontCascade alongside CloudFront-specific fields.
	globalKropathMandatoryCF := globalKropath.Spec.Mandatory.CloudFront
	globalKropathMandatoryCF.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCF := localKropath.Spec.Mandatory.CloudFront
	localKropathMandatoryCF.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCF := localKropath.Spec.Defaults.CloudFront
	localKropathDefaultsCF.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCF := globalKropath.Spec.Defaults.CloudFront
	globalKropathDefaultsCF.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCloudFrontCascade(
		globalKropathMandatoryCF,   // level 1
		localKropathMandatoryCF,    // level 2
		globalCFCfg.Spec.Mandatory, // level 3
		cfg.Spec.Mandatory,         // level 4
		cfg.Spec.Defaults,          // level 6
		globalCFCfg.Spec.Defaults,  // level 7
		localKropathDefaultsCF,     // level 8
		globalKropathDefaultsCF,    // level 9
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCFCfgFound, globalCFCfgViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCloudFrontConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CloudFrontConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCloudFrontConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.CloudFrontConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudFrontConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForCloudFrontConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CloudFrontConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.CloudFrontConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CloudFrontConfig configs for CloudFrontConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list CloudFrontConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
