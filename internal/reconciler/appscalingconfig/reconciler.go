...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.AppScalingConfig{}).
		Watches(
			&v1alpha1.AppScalingConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForAppScalingConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.AppScalingConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Promote tier-level tags from KropathConfigTier.Tags into each AppScalingKropathSection
	// so that tag cascade flows through MergeAppScalingCascade (same pattern as AutoScaling).
	globalKropathMandatoryAS := globalKropath.Spec.Mandatory.AppScaling
	globalKropathMandatoryAS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryAS := localKropath.Spec.Mandatory.AppScaling
	localKropathMandatoryAS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsAS := localKropath.Spec.Defaults.AppScaling
	localKropathDefaultsAS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsAS := globalKropath.Spec.Defaults.AppScaling
	globalKropathDefaultsAS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeAppScalingCascade(
		globalKropathMandatoryAS,   // level 1
		localKropathMandatoryAS,    // level 2
		globalASCfg.Spec.Mandatory, // level 3
		cfg.Spec.Mandatory,         // level 4
		cfg.Spec.Defaults,          // level 6
		globalASCfg.Spec.Defaults,  // level 7
		localKropathDefaultsAS,     // level 8
		globalKropathDefaultsAS,    // level 9
	)

	// AC-12: InvalidCapacityRange validation (spec §AC-12).
	// When both minCapacity > 0 AND maxCapacity > 0 AND min > max,
	// set Valid=False and do NOT write effectiveConfig.
	mandMin := eff.Mandatory.MinCapacity
	mandMax := eff.Mandatory.MaxCapacity
	if mandMin > 0 && mandMax > 0 && mandMin > mandMax {
		invalidCond := metav1.Condition{
			Type:               "Valid",
			Status:             metav1.ConditionFalse,
			Reason:             "InvalidCapacityRange",
			Message:            fmt.Sprintf("mandatory.minCapacity (%d) must not exceed mandatory.maxCapacity (%d)", mandMin, mandMax),
			ObservedGeneration: cfg.Generation,
			LastTransitionTime: now,
		}
		if !conditionNeedsUpdate(cfg.Status.Conditions, invalidCond) {
			return false, ctrl.Result{}, nil
		}
		cfg.Status.Conditions = setCondition(cfg.Status.Conditions, invalidCond)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.AppScalingConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveAppScalingConfig
...
func (r *Reconciler) requestsForAppScalingConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.AppScalingConfig)
	if !ok {
		return nil
	}
