...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.WAFConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("WAFConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.WAFConfig{}).
		Watches(
			&v1alpha1.WAFConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForWAFConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.WAFConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalWAF, globalWAFFound, globalWAFViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.WAFConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("WAFConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each WAFKropathSection with the tier-level tags so that tag cascade
	// flows through MergeWAFCascade alongside WAF-specific fields.
	globalKropathMandatoryWAF := globalKropath.Spec.Mandatory.WAF
	globalKropathMandatoryWAF.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryWAF := localKropath.Spec.Mandatory.WAF
	localKropathMandatoryWAF.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsWAF := localKropath.Spec.Defaults.WAF
	localKropathDefaultsWAF.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsWAF := globalKropath.Spec.Defaults.WAF
	globalKropathDefaultsWAF.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeWAFCascade(
		globalKropathMandatoryWAF,
		localKropathMandatoryWAF,
		globalWAF.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalWAF.Spec.Defaults,
		localKropathDefaultsWAF,
		globalKropathDefaultsWAF,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalWAFFound, globalWAFViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveWAFConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.WAFConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveWAFConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.WAFConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list WAFConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForWAFConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.WAFConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.WAFConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list WAFConfig configs for WAFConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list WAFConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
