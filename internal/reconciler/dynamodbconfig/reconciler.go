...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.DynamoDBConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("DynamoDBConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.DynamoDBConfig{}).
		Watches(
			&v1alpha1.DynamoDBConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForDynamoDBConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.DynamoDBConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalDDB, globalDDBFound, globalDDBViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.DynamoDBConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("DynamoDBConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each DynamoDBKropathSection with the tier-level tags so they flow
	// through MergeDynamoDBCascade alongside DynamoDB-specific fields.
	globalKropathMandatoryDDB := globalKropath.Spec.Mandatory.DynamoDB
	globalKropathMandatoryDDB.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryDDB := localKropath.Spec.Mandatory.DynamoDB
	localKropathMandatoryDDB.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsDDB := localKropath.Spec.Defaults.DynamoDB
	localKropathDefaultsDDB.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsDDB := globalKropath.Spec.Defaults.DynamoDB
	globalKropathDefaultsDDB.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeDynamoDBCascade(
		globalKropathMandatoryDDB,
		localKropathMandatoryDDB,
		globalDDB.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalDDB.Spec.Defaults,
		localKropathDefaultsDDB,
		globalKropathDefaultsDDB,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalDDBFound, globalDDBViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveDynamoDBConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.DynamoDBConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveDynamoDBConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.DynamoDBConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list DynamoDBConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForDynamoDBConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.DynamoDBConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.DynamoDBConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list DynamoDBConfig configs for DynamoDBConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list DynamoDBConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
