...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.MQConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("MQConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.MQConfig{}).
		Watches(
			&v1alpha1.MQConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForMQConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.MQConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalMQ, globalMQFound, globalMQViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.MQConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("MQConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each MQKropathSection with the tier-level tags so they flow
	// through MergeMQCascade (mirrors the cloudwatch tag augmentation pattern).
	globalKropathMandatoryMQ := globalKropath.Spec.Mandatory.MQ
	globalKropathMandatoryMQ.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryMQ := localKropath.Spec.Mandatory.MQ
	localKropathMandatoryMQ.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsMQ := localKropath.Spec.Defaults.MQ
	localKropathDefaultsMQ.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsMQ := globalKropath.Spec.Defaults.MQ
	globalKropathDefaultsMQ.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeMQCascade(
		globalKropathMandatoryMQ,
		localKropathMandatoryMQ,
		globalMQ.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalMQ.Spec.Defaults,
		localKropathDefaultsMQ,
		globalKropathDefaultsMQ,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalMQFound, globalMQViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveMQConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.MQConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveMQConfig
...
func (r *Reconciler) requestsForMQConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.MQConfig)
	if !ok {
		return nil
	}
