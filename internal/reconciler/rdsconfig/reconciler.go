...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.RDSConfig) (bool, ctrl.Result, error) {
	...
	globalRDS, err := r.loadRDSConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list RDS configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list RDS configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForRDSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.RDSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list RDS configs for global RDSConfig change")
		return nil
	}
