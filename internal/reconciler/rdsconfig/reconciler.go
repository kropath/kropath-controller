...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.RDSConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("RDSConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.RDSConfig{}).
		Watches(
			&v1alpha1.RDSConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForRDSConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.RDSConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalRDS, globalRDSFound, globalRDSViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.RDSConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("RDSConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each RDSKropathSection with tier-level tags so they flow
	// through MergeRDSCascade (same pattern as ELB/KMS tag augmentation).
	globalKropathMandatoryRDS := globalKropath.Spec.Mandatory.RDS
	globalKropathMandatoryRDS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryRDS := localKropath.Spec.Mandatory.RDS
	localKropathMandatoryRDS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsRDS := localKropath.Spec.Defaults.RDS
	localKropathDefaultsRDS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsRDS := globalKropath.Spec.Defaults.RDS
	globalKropathDefaultsRDS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeRDSCascade(
		globalKropathMandatoryRDS,
		localKropathMandatoryRDS,
		globalRDS.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalRDS.Spec.Defaults,
		localKropathDefaultsRDS,
		globalKropathDefaultsRDS,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalRDSFound, globalRDSViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveRDSConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.RDSConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveRDSConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.RDSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list RDSConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForRDSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.RDSConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.RDSConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list RDSConfig configs for RDSConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list RDSConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
