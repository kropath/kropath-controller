...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.NetworkFirewallConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("NetworkFirewallConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.NetworkFirewallConfig{}).
		Watches(
			&v1alpha1.NetworkFirewallConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForNetworkFirewallConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.NetworkFirewallConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalNFW, globalNFWFound, globalNFWViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.NetworkFirewallConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("NetworkFirewallConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each NetworkFirewallKropathSection with the tier-level tags so they flow
	// through MergeNetworkFirewallCascade alongside the family-specific fields.
	globalKropathMandatoryNFW := globalKropath.Spec.Mandatory.NetworkFirewall
	globalKropathMandatoryNFW.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryNFW := localKropath.Spec.Mandatory.NetworkFirewall
	localKropathMandatoryNFW.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsNFW := localKropath.Spec.Defaults.NetworkFirewall
	localKropathDefaultsNFW.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsNFW := globalKropath.Spec.Defaults.NetworkFirewall
	globalKropathDefaultsNFW.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeNetworkFirewallCascade(
		globalKropathMandatoryNFW,
		localKropathMandatoryNFW,
		globalNFW.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalNFW.Spec.Defaults,
		localKropathDefaultsNFW,
		globalKropathDefaultsNFW,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalNFWFound, globalNFWViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveNetworkFirewallConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.NetworkFirewallConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveNetworkFirewallConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.NetworkFirewallConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list NetworkFirewallConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNetworkFirewallConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.NetworkFirewallConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.NetworkFirewallConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list NetworkFirewallConfig configs for NetworkFirewallConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list NetworkFirewallConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
