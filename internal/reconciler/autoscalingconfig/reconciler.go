...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.AutoScalingConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("AutoScalingConfig"))
...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&v1alpha1.AutoScalingConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForAutoScalingConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.AutoScalingConfig) (bool, ctrl.Result, error) {
	...
	globalASCfg, err := r.loadAutoScalingConfig(ctx, kroSystemNamespace, cfg.Name)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Promote tier-level tags from KropathConfigTier.Tags into each AutoScalingKropathSection
	// so that tag cascade flows through MergeAutoScalingCascade (same pattern as KMS AC-15).
	globalKropathMandatoryAS := globalKropath.Spec.Mandatory.AutoScaling
	globalKropathMandatoryAS.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryAS := localKropath.Spec.Mandatory.AutoScaling
	localKropathMandatoryAS.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsAS := localKropath.Spec.Defaults.AutoScaling
	localKropathDefaultsAS.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsAS := globalKropath.Spec.Defaults.AutoScaling
	globalKropathDefaultsAS.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeAutoScalingCascade(
	...
	newEffConfig := v1alpha1.EffectiveAutoScalingConfig{
...
func (r *Reconciler) loadAutoScalingConfig(ctx context.Context, namespace, name string) (*v1alpha1.AutoScalingConfig, error) {
	cfg := &v1alpha1.AutoScalingConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("AutoScalingConfig"))
	if err := r.Client.Get(ctx, types.NamespacedName{Namespace: namespace, Name: name}, cfg); err != nil {
		if client.IgnoreNotFound(err) == nil {
			return &v1alpha1.AutoScalingConfig{}, nil
		}
		return nil, err
	}
	return cfg, nil
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if cfg.Namespace == kroSystemNamespace {
		if err := r.Client.List(ctx, &list); err != nil {
			r.Log.Error(err, "unable to list AutoScaling configs for global KropathConfig change")
			return nil
		}
	} else {
		if err := r.Client.List(ctx, &list, client.InNamespace(cfg.Namespace)); err != nil {
			r.Log.Error(err, "unable to list AutoScaling configs for namespace KropathConfig change")
			return nil
		}
	}
...
func (r *Reconciler) requestsForAutoScalingConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	cfg, ok := obj.(*v1alpha1.AutoScalingConfig)
	...
	var list v1alpha1.AutoScalingConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list AutoScaling configs for global AutoScalingConfig change")
		return nil
	}
