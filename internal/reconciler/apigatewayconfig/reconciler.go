...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.APIGatewayConfig) (bool, ctrl.Result, error) {
	...
	// Augment each ApiGatewayKropathSection with the tier-level tags so they flow
	// through MergeApiGatewayCascade (mirrors the EventBridge tag augmentation pattern).
	globalKropathMandatoryApigw := globalKropath.Spec.Mandatory.ApiGateway
	globalKropathMandatoryApigw.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryApigw := localKropath.Spec.Mandatory.ApiGateway
	localKropathMandatoryApigw.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsApigw := localKropath.Spec.Defaults.ApiGateway
	localKropathDefaultsApigw.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsApigw := globalKropath.Spec.Defaults.ApiGateway
	globalKropathDefaultsApigw.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeApiGatewayCascade(
		globalKropathMandatoryApigw,
		localKropathMandatoryApigw,
		globalApigw.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalApigw.Spec.Defaults,
		localKropathDefaultsApigw,
