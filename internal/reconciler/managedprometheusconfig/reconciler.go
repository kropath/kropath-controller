...
package managedprometheusconfig
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.ManagedPrometheusConfig{}).
		Watches(
			&v1alpha1.ManagedPrometheusConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForManagedPrometheusConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.ManagedPrometheusConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each ManagedPrometheusKropathSection with the tier-level tags so they flow
	// through MergeManagedPrometheusCascade alongside the Managed Prometheus-specific fields.
	globalKropathMandatoryMP := globalKropath.Spec.Mandatory.ManagedPrometheus
	globalKropathMandatoryMP.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryMP := localKropath.Spec.Mandatory.ManagedPrometheus
	localKropathMandatoryMP.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsMP := localKropath.Spec.Defaults.ManagedPrometheus
	localKropathDefaultsMP.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsMP := globalKropath.Spec.Defaults.ManagedPrometheus
	globalKropathDefaultsMP.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeManagedPrometheusCascade(
		globalKropathMandatoryMP,
		localKropathMandatoryMP,
		globalMP.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalMP.Spec.Defaults,
		localKropathDefaultsMP,
		globalKropathDefaultsMP,
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.ManagedPrometheusConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveManagedPrometheusConfig
...
func (r *Reconciler) requestsForManagedPrometheusConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.ManagedPrometheusConfig)
	if !ok {
		return nil
	}
