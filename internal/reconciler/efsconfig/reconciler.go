...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.EFSConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("EFSConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.EFSConfig{}).
		Watches(
			&v1alpha1.EFSConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForEFSConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.EFSConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalEFS, globalEFSFound, globalEFSViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.EFSConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("EFSConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// EFS has no family-specific KropathConfig scalars — only org-wide tags flow through.
	// Populate EFSKropathSection.Tags from each tier's org-wide tags.
	globalKropathMandatoryEFS := cascade.EFSKropathSection{Tags: globalKropath.Spec.Mandatory.Tags}
	localKropathMandatoryEFS := cascade.EFSKropathSection{Tags: localKropath.Spec.Mandatory.Tags}
	localKropathDefaultsEFS := cascade.EFSKropathSection{Tags: localKropath.Spec.Defaults.Tags}
	globalKropathDefaultsEFS := cascade.EFSKropathSection{Tags: globalKropath.Spec.Defaults.Tags}

	eff := cascade.MergeEFSCascade(
		globalKropathMandatoryEFS,
		localKropathMandatoryEFS,
		globalEFS.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalEFS.Spec.Defaults,
		localKropathDefaultsEFS,
		globalKropathDefaultsEFS,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalEFSFound, globalEFSViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveEFSConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.EFSConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveEFSConfig
...
func (r *Reconciler) requestsForEFSConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.EFSConfig)
	if !ok {
		return nil
	}
