...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.CodeArtifactConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("CodeArtifactConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.CodeArtifactConfig{}).
		Watches(
			&v1alpha1.CodeArtifactConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForCodeArtifactConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.CodeArtifactConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalCA, globalCAFound, globalCAViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.CodeArtifactConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("CodeArtifactConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each CodeArtifactKropathSection with the tier-level tags so they flow
	// through MergeCodeArtifactCascade (mirrors the CloudWatchLogs tag augmentation pattern).
	globalKropathMandatoryCA := globalKropath.Spec.Mandatory.CodeArtifact
	globalKropathMandatoryCA.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryCA := localKropath.Spec.Mandatory.CodeArtifact
	localKropathMandatoryCA.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsCA := localKropath.Spec.Defaults.CodeArtifact
	localKropathDefaultsCA.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsCA := globalKropath.Spec.Defaults.CodeArtifact
	globalKropathDefaultsCA.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeCodeArtifactCascade(
		globalKropathMandatoryCA,
		localKropathMandatoryCA,
		globalCA.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalCA.Spec.Defaults,
		localKropathDefaultsCA,
		globalKropathDefaultsCA,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalCAFound, globalCAViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveCodeArtifactConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.CodeArtifactConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveCodeArtifactConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.CodeArtifactConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CodeArtifactConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForCodeArtifactConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.CodeArtifactConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.CodeArtifactConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list CodeArtifactConfig configs for CodeArtifactConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list CodeArtifactConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
