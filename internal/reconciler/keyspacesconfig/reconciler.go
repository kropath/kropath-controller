...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	cfg := &v1alpha1.KeyspacesConfig{}
	cfg.SetGroupVersionKind(v1alpha1.GroupVersion.WithKind("KeyspacesConfig"))
...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.KeyspacesConfig{}).
		Watches(
			&v1alpha1.KeyspacesConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForKeyspacesConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.KeyspacesConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	globalKeyspaces, globalKeyspacesFound, globalKeyspacesViaFallthrough, err := util.LoadConfigWithFallthrough[v1alpha1.KeyspacesConfig](
		ctx, r.Client, v1alpha1.GroupVersion.WithKind("KeyspacesConfig"), globalNS, cfg.Name, util.DefaultConfigProfile)
	if err != nil {
		return false, ctrl.Result{}, err
	}
	...
	// Augment each KeyspacesKropathSection with the tier-level tags from KropathConfigTier.Tags
	// so they flow through MergeKeyspacesCascade.
	globalKropathMandatoryKeyspaces := globalKropath.Spec.Mandatory.Keyspaces
	globalKropathMandatoryKeyspaces.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryKeyspaces := localKropath.Spec.Mandatory.Keyspaces
	localKropathMandatoryKeyspaces.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsKeyspaces := localKropath.Spec.Defaults.Keyspaces
	localKropathDefaultsKeyspaces.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsKeyspaces := globalKropath.Spec.Defaults.Keyspaces
	globalKropathDefaultsKeyspaces.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeKeyspacesCascade(
		globalKropathMandatoryKeyspaces,
		localKropathMandatoryKeyspaces,
		globalKeyspaces.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalKeyspaces.Spec.Defaults,
		localKropathDefaultsKeyspaces,
		globalKropathDefaultsKeyspaces,
	)
	...
	profileCond := util.ConfigProfileResolvedCondition(cfg.Name, globalKeyspacesFound, globalKeyspacesViaFallthrough, cfg.Generation, now)
	newEffConfig := v1alpha1.EffectiveKeyspacesConfig{
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.KeyspacesConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveKeyspacesConfig
...
func (r *Reconciler) requestsForKeyspacesConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.KeyspacesConfig)
	if !ok {
		return nil
	}
