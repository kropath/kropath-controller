...
func (r *Reconciler) BuildWithManager(mgr ctrl.Manager) (controller.Controller, error) {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.GlueConfig{}).
		Watches(
			&v1alpha1.GlueConfig{},
			handler.EnqueueRequestsFromMapFunc(r.requestsForGlueConfigChange),
		).
		Watches(
...
func (r *Reconciler) reconcile(ctx context.Context, cfg *v1alpha1.GlueConfig) (bool, ctrl.Result, error) {
	now := metav1.Now()
	...
	// Augment each GlueKropathSection with the tier-level tags so that tag cascade
	// flows through MergeGlueCascade alongside Glue-specific fields.
	globalKropathMandatoryGlue := globalKropath.Spec.Mandatory.Glue
	globalKropathMandatoryGlue.Tags = globalKropath.Spec.Mandatory.Tags
	localKropathMandatoryGlue := localKropath.Spec.Mandatory.Glue
	localKropathMandatoryGlue.Tags = localKropath.Spec.Mandatory.Tags
	localKropathDefaultsGlue := localKropath.Spec.Defaults.Glue
	localKropathDefaultsGlue.Tags = localKropath.Spec.Defaults.Tags
	globalKropathDefaultsGlue := globalKropath.Spec.Defaults.Glue
	globalKropathDefaultsGlue.Tags = globalKropath.Spec.Defaults.Tags

	eff := cascade.MergeGlueCascade(
		globalKropathMandatoryGlue,
		localKropathMandatoryGlue,
		globalGlue.Spec.Mandatory,
		cfg.Spec.Mandatory,
		cfg.Spec.Defaults,
		globalGlue.Spec.Defaults,
		localKropathDefaultsGlue,
		globalKropathDefaultsGlue,
	)
...
func (r *Reconciler) withholdEffectiveConfig(cfg *v1alpha1.GlueConfig, reconciledCond metav1.Condition, placementCond *metav1.Condition) bool {
	...
	var zero v1alpha1.EffectiveGlueConfig
...
func (r *Reconciler) requestsForKropathConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	var list v1alpha1.GlueConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list GlueConfig configs for KropathConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForGlueConfigChange(ctx context.Context, obj client.Object) []ctrl.Request {
	trigger, ok := obj.(*v1alpha1.GlueConfig)
	if !ok {
		return nil
	}
	...
	var list v1alpha1.GlueConfigList
	if err := r.Client.List(ctx, &list); err != nil {
		r.Log.Error(err, "unable to list GlueConfig configs for GlueConfig change")
		return nil
	}
...
func (r *Reconciler) requestsForNamespaceChange(ctx context.Context, obj client.Object) []ctrl.Request {
	...
	if err := r.Client.List(ctx, &list, client.InNamespace(ns.Name)); err != nil {
		r.Log.Error(err, "unable to list GlueConfig configs for namespace change", "namespace", ns.Name)
		return nil
	}
